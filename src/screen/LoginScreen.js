    ...
    render() {
        ...
        return (
            <ImageBackground source={Icons.ic_bg} style={styles.container}>
                <View>
                    <View style={{alignSelf: 'flex-start'}}><Text style={styles.lbText}>{StringUtils.login}</Text><Text
                        style={[styles.styleWelcome, {
                            color: '#333333',
                            marginLeft: 40,
                            fontWeight: 'normal'
                        }]}>{StringUtils.welcome}</Text>
                    </View>

                    <View>
                        <View style={{marginTop: Dimens.TOP_DIMEN, marginRight: 40, marginLeft: 40}}><TextInputBase
                            icon={Icons.ic_mail} text={StringUtils.email} etValue={etMail}
                            textChange={(etMail) => {
                                this.setState({etMail})
                            }}/></View>
                        <View style={{marginTop: Dimens.TOP_DIMEN, marginRight: 40, marginLeft: 40}}><TextInputBase
                            icon={Icons.ic_lock} text={StringUtils.password} visible={true} etValue={etPass}
                            textChange={(etPass) => {
                        ...
                        <Text style={[styles.styleWelcome, {
                            textAlign: 'right',
                            marginRight: 40
                        }]}>{StringUtils.forgot_pass}</Text>

                        <Button block success rounded onPress={() => {
                            this._retrieveData()
                        }}
                                style={[styles.styleBtn, {marginRight: 40, marginLeft: 40}]}>
                            <Text style={{textTransform: 'capitalize'}}>{StringUtils.lets_go}</Text>
                        </Button>
                ...
                <View style={[styles.bottom, {alignSelf: 'center'}]}>
                    ...
                    }}><Text style={styles.text}>{StringUtils.creat_account}</Text></TouchableOpacity>
                </View>
